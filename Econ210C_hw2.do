...
*------------------------ Problem 2: Romer Shocks ------------------------------*

***Merge with Romer shock data***
rename quarter date
merge 1:1 date using RR_monetary_shock_quarterly.dta
drop _merge
replace resid_full = 0 if date < 36 // set equal to zero if date < 1969Q1. 
rename date quarter
*NOTE: I might need to replace resid_full with resid_romer.

***Impulse response function from Romer-Romer model***
tsset quarter

var pi_def UNRATE R if insample, lag(1/8) exo(L(0/12).resid_full) // estimate Romer-Romer VAR

cap irf drop v1, set(irf_simple)
irf create v1, step(20) set(irf_simple) replace
irf graph dm, impulse(resid_full) response(pi_def UNRATE R) yline(0,lcolor(black)) xlabel(0(4)20) byopts(yrescale)
graph export "irf2b.png", replace

***IRF for SVAR with RR, pi, u, R
var resid_full pi_def UNRATE R if insample, lag(1/4) // estimate VAR with 4 lags

cap irf drop v1, set(irf_simple)
irf create v1, step(20) set(irf_simple) replace
irf graph oirf, impulse(resid_full pi_def UNRATE R) response(resid_full pi_def UNRATE R) yline(0,lcolor(black)) xlabel(0(4)20) byopts(yrescale)
graph export "irf2c.png", replace
